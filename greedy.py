...
def dense_video_captioning(path, file, search):
    ...
    proc = subprocess.Popen(['conda', 'run', '-n', 'bmt', 'python', 'single_video_scripts/dense_video_captioning.py', path, file, search], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    out, err = proc.communicate()
    out_file=path+search+".txt"
    with open(out_file, 'wb') as f:
        #print('output from proc: ', out)
        f.write(out)
